...
describe('Search API', () => {
  ...
  describe('search suggest', () => {
    it('search suggest artist should return artists', async () => {
      const result = await sdk.search.searchSuggest('Kasabia');

      expect(fetchSpy.request().input).toBe(
        `https://api.music.yandex.net/search/suggest?part=Kasabia`,
      );
